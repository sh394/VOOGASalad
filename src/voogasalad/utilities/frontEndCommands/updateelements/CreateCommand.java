...
public class CreateCommand extends FrontEndCommand {
    ...
    public void execute() {
        try {
            Method m = getEngineController().getClass().getDeclaredMethod(methodName, Integer.TYPE, String.class, String.class, Point2D.Double.class, Double.TYPE, Double.TYPE, Double.TYPE);
            m.invoke(getEngineController(), ID, filePath, type, location, orientation, size, radius);
        } catch (Exception e) {
